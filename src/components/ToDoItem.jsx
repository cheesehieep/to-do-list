...
const ToDoItem = ({ item, complete, update, remove }) => {
  ...
  return (
    <li className="d-flex align-items-center list-group-item">
      <button className="btn shadow-none" onClick={() => handleComplete(item.id)}>
        <i className={"fas fa-check-circle" + (item.isComplete ? " text-success" : "")}></i>
      </button>
      {editing ? 
      ...
      : (<button 
          className={"btn flex-grow-1 shadow-none" + (item.isComplete ? " completed" : "")} 
